...
verificar_tamanio() {
    ...
    if [ "$tamanio_mb" -lt "$MIN_TAMANIO_MB" ]; then
        log "WARNING" \
            "Directorio pequeño: ${tamanio_mb} MB (mínimo: ${MIN_TAMANIO_MB} MB)"
        return 0
    fi

    if [ "$tamanio_mb" -gt "$MAX_TAMANIO_MB" ]; then
        log "WARNING" \
            "Directorio grande: ${tamanio_mb} MB (máximo: ${MAX_TAMANIO_MB} MB)"
        return 0
    fi
